...
/**
 * @ngdoc function
 * @name moveupApp.controller:announcementsCtrl
 * @description
 * # rise
 * Controller of the rise
 */
// $scopeはJS―HTMLデータ共有する  デペンデンシー (Dependency)
//$routeParams，urlのパラメータを獲得   デペンデンシー (Dependency)
//'$location' URL 
angular.module('moveupApp')
  .controller('AnnouncementsCtrl', ['$scope', '$http','ApiService', '$location', '$routeParams', 'ngDialog', '$timeout',function ($scope, $http, apiService,$location,$routeParams,ngDialog,$timeout) {
    ...
    $scope.getData = function () {
      ...
      apiService.getAnnouncementsList($routeParams.type,$scope.pageSize, $scope.pageSize * ($routeParams.page-1),function onSuccess(resultList) {
        $scope.results = resultList;
        if(resultList.announcementsList.length < 2){
          $scope.ad = '<div id="ad" class="ad displayPc" style="" align="center"></div>';
        }

        // ページ数を精算
        var pageCount = $scope.results.announcementsListCount / $scope.pageSize;
        ...
        if($routeParams.page > 1){
          $scope.lastPage = '/announcements/' + $routeParams.type + '/' + (parseInt($routeParams.page) - 1) + '/';
        }
        if($routeParams.page < totalPage){
          $scope.nextPage = '/announcements/' + $routeParams.type + '/' + (parseInt($routeParams.page) + 1) + '/';
        }
        // var pageCount = $scope.results.announcementsListCount / $scope.pageObject.pageSize;
    ...
    $scope.initPage = function(totalPage){
      ...
      for(var i=0;i<totalPage;i++){
        ...
        var htmlCode = '<a href="/announcements/'+$routeParams.type+'/'+ (i + 1) + '/">' +
